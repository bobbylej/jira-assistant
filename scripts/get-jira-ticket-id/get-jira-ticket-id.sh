...
# Use case-insensitive grep with -i flag instead of (?i) syntax
REGEX_PATTERN="${JIRA_PROJECT_KEY}-[0-9]+"

# Search for Jira ticket in PR title first, then in branch name
TICKET_ID=""

if [ -n "$PR_TITLE" ]; then
    TICKET_ID=$(echo "$PR_TITLE" | grep -ioE "$REGEX_PATTERN" | head -1)
fi

# If not found in PR title, search in branch name
if [ -z "$TICKET_ID" ] && [ -n "$BRANCH_NAME" ]; then
    TICKET_ID=$(echo "$BRANCH_NAME" | grep -ioE "$REGEX_PATTERN" | head -1)
fi

# Convert to uppercase for consistency
if [ -n "$TICKET_ID" ]; then
    TICKET_ID=$(echo "$TICKET_ID" | tr '[:lower:]' '[:upper:]')
    echo "Found Jira ticket: $TICKET_ID" >&2
    echo "$TICKET_ID"
else
    echo "No Jira ticket found in PR title or branch name" >&2
    echo ""
fi
